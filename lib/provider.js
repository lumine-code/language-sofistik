...
class SofistikKeywordsContext {
  ...
  getModuleNames() {
    const keywords = this.getKeywords();
    const moduleNames = Object.keys(keywords);
    for (const [alias, target] of Object.entries(this.base.moduleAliases)) {
      if (moduleNames.includes(target) && !moduleNames.includes(alias)) moduleNames.push(alias);
    }
    return moduleNames;
  }
  ...
  getCommandKeywords(moduleName, commandName) {
    const module = this.getModuleKeywords(moduleName);
    const normalizedCommand = String(commandName).toUpperCase();
    return module && module[normalizedCommand] ? module[normalizedCommand] : null;
  }
  ...
  getParamEnums(moduleName, commandName, paramName) {
    ...
    if (params && normalizedParam in params) {
      return params[normalizedParam];
    }
    return null;
  }
...
class SofistikKeywordsProvider {
  ...
  loadMeta() {
    ...
    try {
      if (fs.existsSync(metaPath)) {
        ...
        this.moduleAliases = this._meta.moduleAliases || {};
        this.defaultVersion =
    ...
    this._meta = { versions: [], moduleAliases: {} };
    return this._meta;
  }
  ...
  /**
   * Normalize a public CADINP module name to the identifier used in .err data.
   * @param {string} moduleName - Public name or schema identifier
   * @returns {string} Uppercase schema identifier
   */
  normalizeModuleName(moduleName) {
    const normalized = String(moduleName).trim().toUpperCase();
    return this.moduleAliases[normalized] || normalized;
  }
